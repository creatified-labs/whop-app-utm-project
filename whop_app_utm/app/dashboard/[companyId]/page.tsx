...
function MetricCardContent({
	...
	// Generate mock bar chart data (8 bars for last 8 periods)
	const generateBarChartData = () => {
		const bars = 8;
		const trend = config.trend ?? 0;

		// Use a deterministic wave function instead of Math.random so that
		// server-rendered HTML matches the client during hydration.
		// Different widget titles get slightly different phase offsets so the
		// shapes are not identical.
		const phaseOffset =
			title === "MRR" ? 0 : title === "Total Stats" ? 1 : title === "Metric" ? 2 : 3;

		return Array.from({ length: bars }, (_, i) => {
			const x = (i / Math.max(bars - 1, 1)) * Math.PI * 1.2 + phaseOffset;
			const wave = 0.18 * Math.sin(x); // between roughly -0.18 and +0.18
			const trendFactor = (trend / 100) * (i / Math.max(bars - 1, 1)) * 0.6;

			// Center around 1 so all bars are positive and comparable.
			const value = 1 + wave + trendFactor;
	...
	// Render mini bar chart (height controlled by parent)
	const renderBarChart = () => {
		const trendColor = (config.trend ?? 0) >= 0 ? "#34d399" : "#f87171";

		return (
			<div className="flex items-end gap-[3px] h-16">
				{barData.map((value, i) => {
					const heightPercent = (value / maxBar) * 100;
					return (
						<div
							key={i}
	...
	// MRR Widget - Uses total revenue from overview
	if (isMRR) {
	...
	// Total Stats Widget - Overview breakdown
	if (isTotalStats) {
	...
	// Conversion Widget - Overall conversion rate with bar chart
	...
	// Total Revenue Widget - Revenue with breakdown
	if (isRevenue) {
		const revenueLabel = totalRevenue.toLocaleString(undefined, {
			maximumFractionDigits: 0,
		});
...
export default function DashboardPage() {
	...
	// Overview metrics derived from real Whop/Supabase data via /api/tracking-links.
	...
	const saveDashboardConfig = (nextModules: DashboardModule[], nextLayout: Layout[]) => {
		void (async () => {
			try {
				...
				// Best-effort persistence; ignore errors in UI.
			}
		})();
	};
	...
	const handleChangeWidget = (id: string, nextTitle: string) => {
		// Placeholder: in a future step this will open a widget picker.
		// For now, just swap the title so the user can pick between presets.
	...
	useEffect(() => {
		const handleResize = () => {
			...
			// - >= 1280px (xl): 4 columns (desktop)
			// - 640px–1279px (sm/md/lg): 2 columns (2x2 for four widgets)
			...
			if (isReturningToDesktop && desktopLayoutRef.current) {
				...
				// For tablet and mobile, derive a responsive layout so widgets
				// wrap within the available column count instead of overflowing.
	...
	// Listen for the global "Add widget" button in the header.
	useEffect(() => {
		if (typeof window === "undefined") return;
			...
			<div className="max-w-7xl mx-auto space-y-6">
				<div className="flex justify-end">
					<button
						...
						className="inline-flex items-center gap-2 rounded-2xl border border-border bg-black text-white dark:bg-white dark:text-black px-4 py-2 text-sm font-medium shadow-md hover:shadow-lg transform transition-transform hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed"
							...
							return (
								<div key={module.id}>
									<DashboardCard className="h-full min-h-[160px] flex flex-col relative p-4 sm:p-5">
										...
										{/* Header with title, icon, and menu */}
										<div className="flex items-start justify-between mb-4">
											...
											<div className="relative">
												...
												{menuForId === module.id && (
													<div className="absolute right-0 top-[calc(100%+8px)] z-50 w-44 rounded-xl bg-white dark:bg-[#1a1a1a] shadow-xl text-xs overflow-hidden">
														<div className="py-1">
										...
										{/* Decorative curved resize indicator in bottom-right corner */}
				...
				{isPickerOpen && (
					<div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur-sm">
						<div className="mx-4 w-full max-w-4xl rounded-2xl bg-white dark:bg-[#121212] shadow-xl p-5 sm:p-6">
							<div className="flex items-center justify-between gap-3">
								<div>
									...
									<p className="mt-1 text-[11px] text-muted-foreground">
										Choose a widget to add to your dashboard layout.
									</p>
								</div>
								<button
									type="button"
								...
								{WIDGET_TITLES.map((title) => {
									...
									return (
										<div key={title} className="group relative">
											<DashboardCard className="h-56 flex flex-col relative p-5 overflow-hidden">
												<div className="opacity-100 group-hover:opacity-25 transition-opacity duration-200 h-full flex flex-col">
											...
											<button
												type="button"
												onClick={() => handleAddWidgetOfType(title)}
												className="pointer-events-auto absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
											>
												<span className="inline-flex items-center rounded-full bg-white text-black px-3 py-1 text-xs font-medium shadow-sm">
													+ Add widget
												</span>
											</button>
										</div>
