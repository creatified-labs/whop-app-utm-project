...
export default function TrackingLinksPage({ initialView }: TrackingLinksPageProps) {
	...
	const [viewMode, setViewMode] = useState<"cards" | "table">(() => {
		...
		if (typeof window !== "undefined") {
			try {
				const stored = window.localStorage.getItem("utm_tracking_links_view_mode");
	...
	const [whopLinksRaw, setWhopLinksRaw] = useState<TrackingLink[]>([]);
	...
	useEffect(() => {
		...
		try {
			window.localStorage.setItem("utm_tracking_links_view_mode", viewMode);
		} catch { }
	}, [viewMode]);

	// Fetch both Whop tracking links and advanced links together
	useEffect(() => {
		let cancelled = false;

		async function loadAllLinks() {
			try {
				// Load advanced links first
				const [advRes1, advRes2] = await Promise.all([
					fetch("/api/advanced-links"),
					fetch("/api/advanced-links-data"),
				]);

				let advancedLinksData: TrackingLink[] = [];

				if (advRes1.ok) {
					const data1 = (await advRes1.json()) as { links?: TrackingLink[] };
					if (Array.isArray(data1.links)) {
						advancedLinksData = [...advancedLinksData, ...data1.links];
					}
				}

				if (advRes2.ok) {
					const data2 = (await advRes2.json()) as { links?: TrackingLink[] };
					if (Array.isArray(data2.links)) {
						advancedLinksData = [...advancedLinksData, ...data2.links];
					}
				}

				// Deduplicate advanced links
				const uniqueAdvancedLinks = Array.from(
					new Map(advancedLinksData.map((link) => [link.id, link])).values()
				);

				// Now load Whop links
				const whopRes = await fetch("/api/tracking-links");
				if (!whopRes.ok) throw new Error("Failed to load Whop tracking links");
				const whopData = await whopRes.json();

				if (cancelled) return;

				// Set advanced links first, then Whop links
				setAdvancedLinks(uniqueAdvancedLinks);
				setWhopLinksRaw(Array.isArray(whopData.links) ? whopData.links : []);
				setIsLoadingLinks(false);
			} catch (error) {
				console.error("[TrackingLinksPage] Failed to load links:", error);
				if (cancelled) return;
				setWhopLinksRaw([]);
				setIsLoadingLinks(false);
			}
	...
	// Filter out any links that also exist in advanced links (by matching name, slug, or ID pattern)
	const whopLinks = useMemo(() => {
		// Create sets of advanced link identifiers for fast lookup
		const advancedLinkNames = new Set(advancedLinks.map((link) => link.name.toLowerCase()));
		const advancedLinkSlugs = new Set(advancedLinks.map((link) => link.slug?.toLowerCase()).filter(Boolean));
		const advancedLinkIds = new Set(advancedLinks.map((link) => link.id));

		return whopLinksRaw.filter((link) => {
			// Exclude if this link's ID matches an advanced link ID
			if (advancedLinkIds.has(link.id)) {
				return false;
			}
			// Exclude if this link's name matches an advanced link
			if (advancedLinkNames.has(link.name.toLowerCase())) {
				return false;
			}
			// Exclude if this link's slug matches an advanced link
			if (link.slug && advancedLinkSlugs.has(link.slug.toLowerCase())) {
				return false;
			}
			// Exclude links that have advanced link ID pattern (adv_*)
			if (link.id.startsWith("adv_")) {
				return false;
			}
			return true;
		});
	}, [whopLinksRaw, advancedLinks]);
